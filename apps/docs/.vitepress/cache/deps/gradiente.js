...
// node_modules/gradiente/dist/index.mjs
function roundTo(value, digits2) {
  const factor2 = 10 ** digits2;
  return Math.round(value * factor2) / factor2;
}
function floorTo(value, digits2) {
  const factor2 = 10 ** digits2;
  return Math.floor(value * factor2) / factor2;
}
function ceilTo(value, digits2) {
  const factor2 = 10 ** digits2;
  return Math.ceil(value * factor2) / factor2;
}
function truncTo(value, digits2) {
  const factor2 = 10 ** digits2;
  return Math.trunc(value * factor2) / factor2;
}
...
function normalizeAngleDeg(value, digits2 = 3) {
  return roundTo((value % 360 + 360) % 360, digits2);
}
...
function toAngleRad(value, unit) {
  switch (unit) {
    ...
    default:
      throw new Error(`Unsupported angle unit: ${String(unit)}`);
  }
}
function normalizeAngle(value, unit, digits2 = 6) {
  return roundTo(normalizeAngleRad(toAngleRad(value, unit)), digits2);
}
var PatternTokenKind = function(PatternTokenKind2) {
...
var REPEATING_PREFIX = "repeating-";
var PARAMS_VALIDATION_PATTERN = "^[([config,color-stop,([color-hint,color-stop]|color-stop)]|color-stop),~([color-hint,color-stop]|color-stop)].";
...
function isColorStop(value) {
  try {
    ...
    return parse_default(chunk) !== void 0;
  } catch {
    return false;
  }
}
...
var GradientBase = class {
  constructor(options) {
    __publicField(this, "_isRepeating");
...
var LinearGradient = class LinearGradient2 extends GradientBase {
  constructor(config) {
    super(config);
    __publicField(this, "type", "linear-gradient");
  }
  static normalizeConfig(value) {
    if (typeof value === "string") {
      ...
      if (hasTop && hasLeft) return { angle: degToRad2(315) };
      else if (hasTop && hasRight) return { angle: degToRad2(45) };
      else if (hasBottom && hasLeft) return { angle: degToRad2(225) };
      else if (hasBottom && hasRight) return { angle: degToRad2(135) };
      else if (hasTop) return { angle: degToRad2(0) };
      else if (hasRight) return { angle: degToRad2(90) };
      else if (hasBottom) return { angle: degToRad2(180) };
      else if (hasLeft) return { angle: degToRad2(270) };
      throw new SyntaxError(`Unsupported linear gradient keyword direction: "${value}"`);
    }
    switch (value.unit) {
      case "deg":
        return { angle: normalizeAngleRad(degToRad2(value.value)) };
      case "rad":
  ...
  static fromString(input) {
    return LinearGradient2.fromAbi(parseStringToAbi(input));
  }
  static fromAbi(abi) {
    var _a3;
    let config = { angle: 0 };
    if (abi.inputs[0].type === "config") {
      ...
      if (inputValue.startsWith("to ")) config = LinearGradient2.normalizeConfig(inputValue);
      else {
        ...
        config = LinearGradient2.normalizeConfig({
          value: rawValue,
          unit
        });
      }
    }
    const inputsWithoutConfig = ((_a3 = abi.inputs[0]) == null ? void 0 : _a3.type) === "config" ? abi.inputs.slice(1) : abi.inputs;
    const stops = LinearGradient2._normalizeAbiInputsToStops(inputsWithoutConfig);
  ...
  clone() {
    return new LinearGradient2(this.toJSON());
  }
  toString() {
    return `${this.isRepeating ? `repeating-${this.type}` : this.type}(${[this.config.angle === 0 ? "" : `${roundTo(radToDeg2(this.config.angle), 3)}deg`, ...this._serializeStopsCompact()].filter(Boolean).join(", ")})`;
  }
  _validateConfig(_) {
  }
};
var RadialGradient = class RadialGradient2 extends GradientBase {
  constructor(config) {
    super(config);
    __publicField(this, "type", "radial-gradient");
  }
  static fromString(input) {
    return RadialGradient2.fromAbi(parseStringToAbi(input));
  }
  static fromAbi(abi) {
    var _a3;
  ...
  clone() {
    return new RadialGradient2(this.toJSON());
  }
  toString() {
...
var ConicGradient = class ConicGradient2 extends GradientBase {
  constructor(config) {
    super(config);
    __publicField(this, "type", "conic-gradient");
  }
  clone() {
    return new ConicGradient2(this.toJSON());
  }
  toString() {
  ...
  static fromAbi(abi) {
    ...
    const config = this._parseConfig(configInput == null ? void 0 : configInput.value);
    const stops = this._normalizeAbiInputsToStops(inputsWithoutConfig);
...
var ModuleTransformerLinearGradientToCss = class {
  constructor() {
    __publicField(this, "target", "css");
    __publicField(this, "gradientType", "linear-gradient");
  }
  to(input) {
...
var ModuleTransformerRadialGradientToCss = class {
  constructor() {
    __publicField(this, "target", "css");
    __publicField(this, "gradientType", "radial-gradient");
  }
  to(input) {
...
var ModuleTransformerConicGradientToCss = class {
  constructor() {
    __publicField(this, "target", "css");
    __publicField(this, "gradientType", "conic-gradient");
  }
  to(input) {
...
var toRgb$2 = converter_default("rgb");
...
var ModuleTransformerLinearGradientToCanvas = class {
  constructor() {
    __publicField(this, "target", "canvas");
    __publicField(this, "gradientType", "linear-gradient");
  }
  to(input) {
...
var toRgb$1 = converter_default("rgb");
...
var ModuleTransformerConicGradientToCanvas = class {
  ...
  _toRad(angle) {
    if (angle.unit === "deg") return degToRad2(angle.value);
...
var _a;
var GradientTransformer = (_a = class {
  static add(module) {
...
}, __publicField(_a, "_modules", /* @__PURE__ */ new Map()), __publicField(_a, "_initialized", false), _a);
var _a2;
var GradientFactory = (_a2 = class {
...
}, __publicField(_a2, "_registry", /* @__PURE__ */ new Map()), __publicField(_a2, "_initialized", false), _a2);
...
function format(input) {
  if (typeof input === "string") return parse2(input).toString();
  return input.toString();
}
function transformTo(target, input) {
  const gradient = typeof input === "string" ? parse2(input) : input;
...
//# sourceMappingURL=gradiente.js.map
