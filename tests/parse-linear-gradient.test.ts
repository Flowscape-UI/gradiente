...
import { tokenize, parseLinearGradient, degToRad, roundTo } from '../src';


function parseLinear(source: string) {
    const tokens = tokenize(source);
    return parseLinearGradient(source, tokens, 0);
}

describe('parseLinearGradient', () => {
    describe('check basic parsing', () => {
        it('should return correct kind and repeat mode', () => {
            // Check normal linear-gradient
            const value = parseLinear('linear-gradient(red, blue)');
            expect(value.node.kind).toBe('linear');
            expect(value.node.repeat).toBe('normal');

            // Check repeating-linear-gradient
            const repeatingValue = parseLinear('repeating-linear-gradient(red 10%, blue 20%)');
            expect(repeatingValue.node.kind).toBe('linear');
