...
function createNode(value: string, id: string): StructureNode {
  if (value === 'config') return { id, type: 'entity', title: 'CONFIG', symbol: value, description: 'Gradient configuration input.' }
  if (value === 'color-stop') return { id, type: 'entity', title: 'COLOR STOP', symbol: value, description: 'Gradient color stop input.' }
  if (value === 'color-hint') return { id, type: 'entity', title: 'COLOR HINT', symbol: value, description: 'Interpolation hint between color stops.' }

  const map: Record<string, Omit<StructureNode, 'id'>> = {
    '^': { type: 'operator', title: 'BEGIN', symbol: '^', description: 'Pattern starts here.' },
    '.': { type: 'operator', title: 'END', symbol: '.', description: 'Pattern ends here.' },
    '[': { type: 'sequence', title: 'SEQUENCE', symbol: '[', description: 'Ordered list of expressions.', children: [], collapsible: true },
    ']': { type: 'sequence', title: 'SEQUENCE CLOSE', symbol: ']', description: 'Closes the current sequence.' },
    '(': { type: 'group', title: 'GROUP', symbol: '(', description: 'Grouped expression or alternatives.', children: [], collapsible: true },
    ')': { type: 'group', title: 'GROUP CLOSE', symbol: ')', description: 'Closes the current group.' },
    '|': { type: 'operator', title: 'OR', symbol: '|', description: 'Splits the current group into alternative branches.' },
    '~': { type: 'operator', title: 'REPEAT', symbol: '~', description: 'Repeats the next expression zero or more times.' },
    ',': { type: 'separator', title: 'SEPARATOR', symbol: ',', description: 'Separates sequence items.' },
    '!': { type: 'operator', title: 'NOT', symbol: '!', description: 'Reserved logical NOT operator.' },
    '&': { type: 'operator', title: 'AND', symbol: '&', description: 'Reserved logical AND operator.' },
  }

  return { id, ...(map[value] ?? { type: 'unknown', title: 'UNKNOWN', symbol: value, description: 'Unknown token.' }) }
}
...
function explainNode(node: StructureNode, depth = 0): string[] {
  ...
  const children = node.children?.flatMap((child) => explainNode(child, depth + 1)) ?? []
...
const TreeNodeView = defineComponent({
  ...
  props: {
    ...
    onSelectPair: { type: Function as unknown as () => (id: string) => void, required: true },
    onSelectBranches: { type: Function as unknown as () => (ids: string[]) => void, required: true },
    onToggleCollapse: { type: Function as unknown as () => (id: string) => void, required: true },
  },

  setup(props) {
    return () => {
      const theme = tokenTheme[props.node.type]
      const isPairSelected = props.node.pairId && props.node.pairId === props.selectedPairId
      const isBranchSelected = props.node.branchId && props.selectedBranchIds.includes(props.node.branchId)
      ...
      return h('div', { class: 'nodeGroup' }, [
        h(
          ...
          [
            ...
            h('div', { class: 'type', style: typeStyle }, `${props.node.title} = "${props.node.symbol}"`),
...
<template>
  ...
  >
    <TreeNodeView
      ...
      :on-select-pair="(id: string) => { selectedPairId = id; selectedBranchIds = [] }"
      :on-select-branches="(ids: string[]) => { selectedBranchIds = ids; selectedPairId = null }"
