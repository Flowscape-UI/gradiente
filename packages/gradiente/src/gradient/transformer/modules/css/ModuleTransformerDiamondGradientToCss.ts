...
function getColorStops(stops: GradientStop[]): GradientRenderableColorStop[] {
    ...
    for (let index = 0; index < stops.length - 1; index += 1) {
        ...
        if (position >= current.position && position <= next.position) {
            ...
            if (formatted === undefined) {
                throw new Error("Failed to format sampled diamond color.");
            }
...
extends GradientTransformerModule<GradientDiamond, string> {
    ...
    protected transform(gradient: GradientDiamond): string {
        ...
        const center = resolvePosition(config.position);
